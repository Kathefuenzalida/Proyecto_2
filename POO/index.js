...
encuentre entre uno y el máx de alternativas definido previamente para las preguntas*/
  ...
  mostrarResultados(encuesta, respuestas) {
    let mensaje = `📋 Resultados de la encuesta "${encuesta.titulo}":\n\n`;
  
    for (let i = 0; i < encuesta.preguntas.length; i++) {
      const pregunta = encuesta.preguntas[i];
      const respuestaIndex = respuestas[i] - 1; // Porque el usuario eligió desde 1
      const respuestaTexto = pregunta.alternativas[respuestaIndex];
  
      mensaje += `Pregunta ${i + 1}: ${pregunta.texto}\nRespuesta: ${respuestaTexto}\n\n`;
    }
